...
data Card = A | K | Q | J | T | F9 | F8 | F7 | F6 | F5 | F4 | F3 | F2 deriving (Eq, Ord, Enum, Show)
...
handType hand
  | cardCounts == [5] = FiveOfKind
  | cardCounts == [4, 1] = FourOfKind
  | cardCounts == [3, 2] = FullHouse
  | cardCounts == [3, 1, 1] = ThreeOfKind
  | cardCounts == [2, 2, 1] = TwoPair
  | head cardCounts == 2 = OnePair
  | otherwise = HighCard
  where
    cardCounts = sortBy (comparing Data.Ord.Down) (map length $ group $ sort (cards hand))
